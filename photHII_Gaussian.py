...
# -*- coding: utf-8 -*-
"""
A fork of HIIGaussians.py by Jim Acosta.
This version is on Aug 13 2020.

This program allows to identify gaussian emissions on photometric galaxy data.

Parameters: 
            1. A galaxy FITS file in H_alpha or other filter
            2. redshift of galaxy
Returns:
            1. A figure with six different plots of galaxy, gaussian regions
               detected and diffuse regions.
            2. Four FITS images with the information of gaussian and diffuse 
               regions obtained.
            
If you are running Python with Anaconda, you just will need to install
the python package "mpdaf" to get working.

* Original version for cube data; *
Created on Wed May 08 15:35:43 2019
@author: Xavier Jimenez
with silly modification by S.F.Sanchez & C. Espinosa. 19.07.2019
"""
...
# Defining paths (don't forget to put your data on DATA folder)
from CreateFolders import CreateFolders
dirs = ['/DATA/', '/TABLES/', '/IMAGES/']
CreateFolders(dirs)

default_MD_PATH = os.getcwd() + dirs[0]
default_OT_PATH = os.getcwd() + dirs[1]
default_OI_PATH = os.getcwd() + dirs[2]

DATA_PATH = os.getenv('DATA_PATH', default=default_MD_PATH)
...
#def data2FITS(ima, data, name_file):
#    new_header = ima.data_header.copy()
#    for label in ima.data_header.keys():
...
#            if (coeff[0] > F_min) and (fit_errors[3] < 0.08*coeff[3]) and (((size[0]/2-coeff[1])**2 + (size[1]/2-coeff[2])**2)**0.5 < 3) and (coeff[3] < redshift(name)) and (coeff[3]>0.5):
            if (coeff[0] > F_min) and (fit_errors[3] < 0.08*coeff[3]) and (((size[0]/2-coeff[1])**2 + (size[1]/2-coeff[2])**2)**0.5 < 3) and (coeff[3] < scale_based_on_redshift(redshift_input)) and (coeff[3]>0.5):
                ...
                if plot == True:
                    ...
                    gfitim.plot(ax=ax[1], colorbar='v', vmax=np.amax(subima.data), title = r'Simple Gauss2D FIT - flux = %s [10$^{-16}$ cgs]'%round(np.sum(gfitim.data),2))
                    ...
                    res.plot(ax=ax[2],vmin=0, vmax=np.amax(subima.data), colorbar='v', title = r'Residuals - flux = %s [10$^{-16}$ cgs]'%round(np.sum(res.data),2))
                    res.plot(ax=ax[3],vmin=-F_min, vmax=F_min, colorbar='v', title = r'Residuals - flux = %s [10$^{-16}$ cgs]'%round(np.sum(res.data),2))
...
def sigma(ima, plot=False):
    ...
    n, bins, patches = plt.hist(flux, 2000, density= True, facecolor='green', alpha=0.75)
    plt.close()
    bins2=[]
...
def cat(param, name):
    ...
    t.sort('I1')
    t.write(OUTPUT_TABLES_PATH+'table_{}.dat'.format(name), format='ascii', overwrite= True)
    return t
...
#        fig, ax = plt.subplots(2, 3, constrained_layout=True)
        ...
        #res = Image(data=(ima.data - gfitim.data), wcs=ima.wcs)
        gfitim.plot(scale='log', vmin=0, vmax=0.9*np.amax(ima.data), colorbar='v', ax=ax[0,1],   zscale=False, title=r'flux = %s [10$^{-16}$ cgs]'%( np.round(np.sum(gfitim.data),2)))
        gfitim2.plot(scale='log', vmin=0, vmax=0.9*np.amax(ima.data), colorbar='v', ax=ax[0,2],   zscale=False, title=r'flux = %s [10$^{-16}$ cgs]'%( np.round(np.sum(gfitim2.data),2)))
        grid.plot(scale='log', vmin=0, vmax=0.9*np.amax(ima.data), colorbar='v', ax=ax[1,0],   zscale=False, title=r'flux = %s [10$^{-16}$ cgs]'%( np.round(np.sum(grid.data),2)))
        res2 = Image(data=(ima.data - gfitim2.data), wcs=ima.wcs)
...
#F_min
        ...
        res2.plot(scale='log', vmin=0, vmax=0.9*np.amax(ima.data), colorbar='v', ax=ax[1,2],   zscale=False, title='mean = {}, median = {}, std = {}'.format(np.round(np.mean(np.ravel(res2.data)),3), np.round(np.median(np.ravel(res2.data)),3), np.round(np.std(np.ravel(res2.data)),3)))
        fig.tight_layout()
...
#Defining minimum flux manually
#def flux_min(ima, name):
#   std = 0.12631367914366032
#   return 7*std
...
if __name__=='__main__':
    ...
    print('My program took {} hours {} minutes {} seconds to run'.format(hours,minutes,seconds))
